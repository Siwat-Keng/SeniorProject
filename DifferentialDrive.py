...
class DifferentialDrive:
    ...
    def create_robot_motion(self, navigation: Navigation):
        ...
        for idx, path in enumerate(navigation.path):
            if idx+1 < len(navigation.path):
                degree = degrees(atan2(
                    navigation.path[idx+1][1]-navigation.path[idx][1], navigation.path[idx+1][0]-navigation.path[idx][0]))
            self.robot_motion.append(path+(degree, ))
        self.robot_motion.appendleft(
            navigation.current_position+(navigation.start_direction,))
        self.robot_motion[len(self.robot_motion)-1] = (self.robot_motion[len(self.robot_motion)-1][0], 
        self.robot_motion[len(self.robot_motion)-1][1], navigation.final_direction)
